...
> **⏰ Cron frequency & Vercel plans.** The free **Hobby** plan only allows **once-per-day** cron jobs, so `vercel.json` is set to `0 0 * * *` (daily midnight UTC). For more frequent drawdown enforcement:
> - **Pro plan:** change the schedule in `vercel.json` to e.g. `*/15 * * * *` (every 15 min).
> - **Free alternative:** use an external scheduler (e.g. [cron-job.org](https://cron-job.org)) to `GET https://your-site.vercel.app/api/sync` as often as you like, sending header `Authorization: Bearer <CRON_SECRET>`.
